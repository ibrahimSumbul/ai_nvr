...
| 2026-06-24 | **`v1.0-public` referans etiketi + commit-kimliği standardizasyonu** | M0–M7 çekirdek pipeline (dev-stack) + M8 tasarım kontratları noktasında bir referans anlık görüntü etiketlendi. Plan: gerçek kurumsal saha görüntüleriyle değerlendirme + canlı NVR pilotu sonraki faz (görüntüler gizli → ileride ayrı/özel repo). Commit kimliği 3. geçmiş-yeniden-yazımıyla standartlaştırıldı (tek author/committer + `Co-Authored-By`). |
| 2026-06-25 | **Unfreeze — public geliştirme sürüyor + turnkey/taşınabilirlik sağlamlaştırma** | Henüz özel repoya geçilmedi; public geliştirmeye devam kararı (**re-freeze yok**). Çok-ajan saha denetimi: repo `docker pull` ile tek-tık değildi + Linux'ta `host.docker.internal` kopuyordu → `extra_hosts: host-gateway` (bridge+frigate), `make migrate` (`run --rm`), kurulum dokümanı + panel/kamera/healthy sayıları düzeltildi. |

---

## Public yaşam döngüsü (`v1.0-public` referans etiketi, 2026-06)
...
`v1.0-public` etiketi bu noktanın referans anlık görüntüsüdür. Geliştirme bu public repoda **sürüyor**: önce turnkey/taşınabilirlik sağlamlaştırma, ardından uzun soak ve M8 build. Gerçek kurumsal saha görüntüleriyle değerlendirme + canlı Dahua NVR pilotu ileride yapıldığında — görüntüler gizli olacağından — o iş ayrı bir özel repoda yürütülecek; **ama bu geçiş henüz yapılmadı**.

> Dürüstlük notu (ölçülen ≠ çıkarsanan, projenin kendisine uygulanır): bugüne dek tüm doğrulama dev-stack + MediaMTX test stream'leri üzerinde yapıldı; gerçek kurumsal saha görüntüsü değerlendirmesi ve **canlı** Dahua NVR pilotu henüz yapılmadı. İddia buraya kadar, fazlası değil.
